...
# V14 ACTIVITY CANDLE ANALYZER FIX
# Route live 5S activity through CandleAnalyzer so confidence is based on
# the same candle-analysis engine rather than a disconnected score path.
const analyzer = project / 'app/src/main/java/com/example/screener/CandleAnalyzer.kt'
if not analyzer.exists():
    raise SystemExit('V14 CandleAnalyzer missing')
_a14 = analyzer.read_text()
helper = r'''
    // V14: live 5S activity scoring. This is setup quality, not win probability.
    fun quickActivityScore(
        ...
        val bodyRatio = (abs(running.close - running.open) / range).coerceIn(0.0, 1.0)
        val moveRatio = (abs(move) / range).coerceIn(0.0, 1.0)
        val direction = when {
            move > 0.0 -> "CALL"
            move < 0.0 -> "PUT"
        ...
        val recentRange = recent.sumOf { (it.high - it.low).coerceAtLeast(0.0) }.coerceAtLeast(1.0e-9)
        val recentMove = recent.last().close - recent.first().open
        val directionalRecent = if (direction == "CALL") recentMove > 0.0 else recentMove < 0.0
...
'''
if 'fun quickActivityScore(' not in _a14:
    pos=_a14.rfind('
}')
    if pos < 0: raise SystemExit('V14 CandleAnalyzer closing brace not found')
    _a14=_a14[:pos]+"
"+helper+_a14[pos:]
    analyzer.write_text(_a14)

cap = project / 'app/src/main/java/com/example/screener/CaptureService.kt'
if not cap.exists(): raise SystemExit('V14 CaptureService missing')
_cv=cap.read_text()
needle='''            val liveProbability = liveScore.coerceIn(0, 100)
            sendQuickStatus('''
replacement='''            val analyzerQuick =
            ...
            sendQuickStatus('''
if needle not in _cv: raise SystemExit('V14 liveProbability insertion point missing')
_cv=_cv.replace(needle,replacement,1)
_cv=_cv.replace('''                liveDirection,
                liveProbability,''','''                liveDirectionFromAnalyzer,
                liveProbability,''',1)
cap.write_text(_cv)
if 'fun quickActivityScore(' not in analyzer.read_text(): raise SystemExit('V14 analyzer helper missing')
if 'liveDirectionFromAnalyzer' not in cap.read_text(): raise SystemExit('V14 CaptureService bridge missing')
print('V14 FINAL: live 5S activity now scored by CandleAnalyzer')
